...
load('D:\Wind Turbine\MPC\Linearization\ReduceOrder4to3Onshore_2DOF_18_Pch.mat');
A = A_c;
B = [B_c,Bd_c];
C = C_c;
D = [D_c, Dd_c];% 
...
mpcobj.Weights = struct('MV',1e-4,'MVRate',1.2e5,'OV',[4e0 1e1]);%importamt  tuning
% mpcobj.Weights = struct('MV',rou*1e-4,'MVRate',1.2e5,'OV',[(1-rou)*4e0 1e0]);%importamt  tuning
...
e = eig(A1-A1*M*Cm1);
%set new poles
poles = [0.12 0.229 0.296 0.098 0.214];%original estimator pole is 0.0053 0.99. Disctrete pole is less than 1
...
%     alpha = 1;
%     belta = 0.02;%0.99999
%     w = 2.0463*Ts;%0.2 rad/s
%     numT = [1 -2*alpha*cos(w) alpha^2];
%     denT = [1 -2*belta*cos(w) belta^2];
%     sysT = tf(numT,denT,Ts);
    
%     alpha = 0.2; belta = 1; omega = 2.0463;% tower first mode freq. rad/s
%     numT = [1 2*alpha*omega omega^2];
%     denT = [1 2*belta*omega omega^2];
%     sysT = tf(numT,denT);
%     figure;bode(sysT);
    %============================
